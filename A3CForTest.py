...
parser.add_argument('-record_dir',dest='record_dir')
        ...
        while not COORD.should_stop() and GLOBAL_EP < MAX_GLOBAL_EP:
            ...
            while True:
                ...
                if total_step % self.step == 0 or done:   # update global and assign to local net
                    #if len(buffer_r)!=0 and np.std(buffer_r)!=0:
                    #    buffer_r = (np.array(buffer_r)-np.mean(buffer_r))/np.std(buffer_r)
                    if done:
                        v_s_ = 0   # terminal
                    else:
...
if __name__ == "__main__":
    ...
    with open(LOG_DIR+"/test_detail.txt",'w') as f:
        f.write(str(R))
